...
/* JOIN은 단순히 테이블 붙이는 게 아니라
 * 기준 삼은 테이블의 한 컬럼을 지정해 다른 테이블의 한 컬럼과 같은 행을 찾아
 * 기준 테이블 옆에 한 행씩 붙여나감
 */
...
--> 하동운, 이오리가 누락됨. DEPT_CODE값(NULL)과 같은 값이 DEPT_ID에 존재하지 않아서
 --> 최종결과에서 제외 == INNER JOIN
...
--> JOIN 구문 기준. 왼쪽에 작성된 테이블의 모든행이 RESULT SET에 포함 되도록

-- 오라클 구문
SELECT EMP_NAME, DEPT_TITLE 
FROM EMPLOYEE e , DEPARTMENT d 
WHERE DEPT_CODE = DEPT_ID(+);
-- DEPT_CODE의 값이 DEPT_ID와 일치않아도 결과에 포함
...
--> 모든 경우의 수를 보고싶을 때 빼고는 직접 작성하는 경우 거의 없음
 --> 자연 조인 실패 결과로 교차 조인 결과가 출력됨
  --> 교차 조인 결과 보이면 ... 자연 조인 잘못썼구나
...
-- EMPLOYEE.JOB_CODE (CHAR(2)) / JOB.JOB_CODE (CHAR(2))
 --> 컬럼명, 자료형 모두 일치 ! == NATURAL JOIN의 대상
...
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
--> EMP와 LOC 먼저 비교해야 하는데 LOCATION_ID는 DEP 거임
--> EMPLOYEE에 LOCATION_ID 존재 X
...
--연봉에 보너스포인트를 적용하시오.
SELECT DISTINCT EMP_NAME, JOB_NAME, SALARY, SALARY * 12 + SALARY * 12 * NVL(BONUS, 0) 연봉
FROM EMPLOYEE e, JOB j, SAL_GRADE sg 
WHERE e.JOB_CODE = j.JOB_CODE 
AND SALARY > sg.MIN_SAL;

-- 7.한국(KO)과 일본(JP)에 근무하는 직원들의 
-- 사원명, 부서명, 지역명, 국가명을 조회하시오.
SELECT EMP_NAME 사원명, DEPT_TITLE 부서명, LOCAL_NAME 지역명,
	CASE
		WHEN NATIONAL_CODE = 'KO' THEN '한국'
		WHEN NATIONAL_CODE = 'JP' THEN '일본'
	END 국가명
FROM EMPLOYEE, LOCATION l , DEPARTMENT d 
WHERE DEPT_ID = DEPT_CODE
